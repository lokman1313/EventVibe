...
import { deleteEventPermanent } from "@/lib/action/event";


interface EventDeleteAlartProps {
  eventId: string;
}

export function EventDeleteAlart({ eventId }: EventDeleteAlartProps) {
  ...
  const handleDelete = async () => {
    setIsDeleting(true);
    try {
      const res = await deleteEventPermanent(eventId);
      if (res) {
        toast.warning("Event deleted permanently");
        router.refresh();
        setIsOpen(false); // Action successful hole dynamic modal-ti close hobe
      } else {
      ...
      <AlertDialog.Backdrop className="bg-black/50 backdrop-blur-sm">
        <AlertDialog.Container>
          <AlertDialog.Dialog className="sm:max-w-[400px] border border-gray-200 dark:border-white/10 bg-white dark:bg-[#121214] text-gray-900 dark:text-white rounded-2xl shadow-2xl p-6 mx-4">
            ...
            {/* Header with HeroUI Warning Status Icon */}
            ...
            <AlertDialog.Body className="text-center px-2 py-4">
              <p className="text-sm text-gray-500 dark:text-zinc-400 leading-relaxed">
                ...
                <span className="font-semibold text-red-500">cannot be undone</span> and all related data will be cleared.
              </p>
            </AlertDialog.Body>
...
export default EventDeleteAlart;